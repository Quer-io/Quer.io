...
class Model:
    def __init__(self, db_string):
        self.tree = sklearn.tree.DecisionTreeRegressor(criterion='mse')
        ...
        self.accuracy_score = tree.score(test['age'], test['income'])

    def predict(self, age):
        """Returns a tuple with (mean, variance)"""
        node_index = self.tree.apply([age])[0]
        return (tree.tree_.value[node_index], tree.tree_.impurity[node_index])
